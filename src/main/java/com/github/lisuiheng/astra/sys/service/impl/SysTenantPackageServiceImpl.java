...
import com.github.lisuiheng.astra.sys.domain.bo.SysTenantPackageBo;
import com.github.lisuiheng.astra.sys.domain.vo.SysTenantPackageVo;
import com.github.lisuiheng.astra.sys.mapper.SysTenantMapper;
import com.github.lisuiheng.astra.sys.mapper.SysTenantPackageMapper;
import com.github.lisuiheng.astra.sys.service.ISysTenantPackageService;
...
public class SysTenantPackageServiceImpl implements ISysTenantPackageService {
    ...
    public Boolean insertByBo(SysTenantPackageBo bo) {
        ...
        add.setMenuIds(CollUtil.isNotEmpty(menuIds) ? StringUtils.joinComma(menuIds.stream().map(String::valueOf).collect(Collectors.toList())) : "");
    ...
    public Boolean updateByBo(SysTenantPackageBo bo) {
        ...
        // 保存菜单id
        List<Long> menuIds = Arrays.asList(bo.getMenuIds());
        update.setMenuIds(CollUtil.isNotEmpty(menuIds) ? StringUtils.joinComma(menuIds.stream().map(String::valueOf).collect(Collectors.toList())) : "");
