...
public class SysClientController extends BaseController {
    ...
    @GetMapping("/{id}")
    public R<SysClientVo> getInfo(@jakarta.validation.constraints.NotNull(message = "主键不能为空")
    ...
    @RepeatSubmit()
    @PostMapping()
    public R<Void> add(@org.springframework.validation.annotation.Validated(com.github.lisuiheng.astra.common.core.validate.AddGroup.class) @RequestBody SysClientBo bo) {
    ...
    @RepeatSubmit()
    @PutMapping()
    public R<Void> edit(@org.springframework.validation.annotation.Validated(com.github.lisuiheng.astra.common.core.validate.EditGroup.class) @RequestBody SysClientBo bo) {
        if (!sysClientService.checkClickKeyUnique(bo)) {
    ...
    @DeleteMapping("/{ids}")
    public R<Void> remove(@org.hibernate.validator.constraints.Length(min = 1, message = "主键不能为空")
                          @PathVariable Long[] ids) {
        return toAjax(sysClientService.deleteWithValidByIds(java.util.List.of(ids), true));
    }
}