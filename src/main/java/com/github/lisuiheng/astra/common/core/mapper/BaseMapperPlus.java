package com.github.lisuiheng.astra.common.core.mapper;
...
public interface BaseMapperPlus<T, V> extends BaseMapper<T> {
    ...
    default <C, P extends IPage<C>> P selectVoPage(IPage<T> page, Wrapper<T> wrapper, Class<C> voClass) {
        ...
        // 注意：必须在查询之后获取分页信息，因为查询会自动填充分页数据
        ...
        if (page instanceof Page) {
            Page<T> originalPage = (Page<T>) page;
            List<OrderItem> orderItems = originalPage.orders();
    ...
    default <C> List<C> selectObjs(Wrapper<T> wrapper, Function<? super Object, C> mapper) {
        // 修复：使用 Java Stream API 替代不存在的 StreamUtils
        List<Object> objects = this.selectObjs(wrapper);
